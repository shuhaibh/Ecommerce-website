...
const ProductCard = ({ product }) => {
  // The actual item details are inside the populated 'item_id' field.
  const item = product.item_id;

  // Guard clause to prevent rendering if item data is missing
  if (!item) {
    return null; 
  }
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-lg border border-gray-200 bg-white dark:bg-gray-800 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
      {/* The link still uses the main product._id */}
